...
```
                    ...
                    │  - Model registry (wersje modeli AI)         │
                    │  - Management API + UI                       │
                    │  - Feature store (offline)                    │
                    │  - Threat-intel feed ingestion                │
                    └───────────────┬───────────────────────────────┘
                                    │ (konfiguracja, modele - push/pull)
                                    ▼
Klient ──▶ [CDN / L3-4 anty-DDoS] ──▶ [Data Plane: AI-WAF] ──▶ Backend/App
                                    │
                                    ├─▶ Rule Engine (sygnatury, CRS-like, regexy)
                                    ├─▶ Feature extraction (per request + per session)
                                    ├─▶ AI Scoring Engine (modele ML lokalne, low-latency)
                                    ├─▶ Decision Engine (łączy reguły + AI wg polityki)
                                    └─▶ Async pipeline: logi, metryki, zdarzenia ──▶ SIEM/SOAR
                                             │
                                             ▼
                                  Feedback loop: analityk oznacza
                                  FP/FN ──▶ dane treningowe ──▶ retraining
```
...
opóźnienia na ścieżkę krytyczną.
